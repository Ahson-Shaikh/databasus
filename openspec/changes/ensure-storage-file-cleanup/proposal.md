...
- **BREAKING**: the internal database-removal listener contract changes shape. `OnBeforeDatabaseRemove` receives the caller's transaction so its listeners can record deletions atomically, and the three existing implementations move to it. The old signature is deleted rather than kept alongside.
...
- **BREAKING**: the workspace-deletion listener contract gains a `context.Context`, because the storage listener now drains cleanup and writes an audit entry. All three implementations move with it.
