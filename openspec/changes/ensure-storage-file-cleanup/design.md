...
| `attempt_count` | Retry count, and the generation that invalidates a stale receipt |
...
`attempt_count` carries two meanings, so the migration and the repository both say so. It schedules retries, and it is the generation number that lets a write receipt detect that cleanup, or another caller, took the file away from its writer.
...
- on success it refreshes `not_before` to `now() + commitWindow` conditionally on `attempt_count` still holding the value it started with, leaves the live set and returns a `WriteReceipt` carrying the row ID and that `attempt_count`;
...
- `ConfirmFileWrites(ctx, tx, receipts)` inside the transaction that makes the backup, WAL, history, manifest or metadata catalog state usable. It deletes each row by `id` conditionally on the receipt's `attempt_count` and fails the transaction unless every receipt matched exactly one row;
...
`DeletionWorker` holds the live-write mutex across its claim statement and passes the set into it: `UPDATE ... SET attempt_count = attempt_count + 1, not_before = now() + attemptTimeout WHERE id IN (SELECT id FROM storage_pending_deletions WHERE not_before <= now() AND id <> ALL(<live writes>) ORDER BY not_before LIMIT <batch> FOR UPDATE SKIP LOCKED) RETURNING *`. It commits the claim, then deletes remotely without holding a database lock. Both completion and rescheduling are conditional on the claimed `attempt_count`: success deletes the row, failure records a sanitized error and a jittered exponential `not_before` capped at a package maximum.
...
- the claiming update is a compare and swap on `attempt_count`, so a receipt issued before the claim can no longer confirm, and a slow attempt that outlived its `attemptTimeout` can neither reschedule nor complete over a newer one.
...
- [`attempt_count` carries two meanings] -> The migration comment, the repository method names and the design state that every event taking a file away from its writer increments it. A separate generation column would add a second number that always moves with the first.
